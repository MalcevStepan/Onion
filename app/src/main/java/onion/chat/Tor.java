...
public class Tor {
	...
	private File tordir, torsrv;
	private Process tor;
	private BufferedReader torreader;
	...
	private void startTorConnection() {
		try {
			...
			log("make dir");
			tordir = new File(context.getFilesDir(), tordirname);
			tordir.mkdirs();

			log("make service");
			torsrv = new File(context.getFilesDir(), torservdir);
			torsrv.mkdirs();
			...
			torreader = new BufferedReader(new InputStreamReader(tor.getInputStream()));
			while (true) {
				String line;
				try {
